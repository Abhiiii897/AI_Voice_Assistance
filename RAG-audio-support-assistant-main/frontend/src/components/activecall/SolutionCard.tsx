...
function SolutionCard({ card }: Readonly<SolutionCardProps>) {
  ...
  const [isExpanded, setIsExpanded] = useState(false); //expandable steps in card fix
  ...
  const colors = categoryColors[card.category] || categoryColors['Machine Operation']; //fallback to first color if not classified

  // Determine how many steps to show (max 3, then "X more")
  const visibleSteps = isExpanded ? card.steps : card.steps.slice(0, 3);
  const hiddenStepCount = card.steps.length - visibleSteps.length; //calculate the no of hidden steps
  ...
  return (
    <div className="bg-gray-700/50 rounded-lg p-3 border border-gray-600 hover:border-gray-500 transition-colors">
      ...
      {/* Source Info (NOTE: Manual pdf referncing to be implemented in the backend for this to work)*/}
